...
export const addUserValidation = (formData: FormData, textmask: string): { isValid: boolean; newErrors: Errors } => {
    let isValid = true;
    ...
    if (formData.regionId === '') {
        newErrors.regionId = "Поле регіону обов'язкове";
        isValid = false;
    }

    if (formData.bankId === '') {
        newErrors.bankId = "Поле банку обов'язкове";
        isValid = false;
    }
