...
import { Checkbox, FormControl, FormControlLabel, FormGroup, MenuItem, TextField, ThemeProvider } from '@mui/material';
import { editUser, GetUserById } from '../../services/accounts/account-services';
                ...
                .then(data => {
                    if (data) {
                        ...
                        setValues((prevValues) => ({
                            ...prevValues,
                            textmask: data.user.phoneNumber ?? '',
                        }));
                        const selectedRole = ROLES.find(role => role.name === data.user.role) || null;
                        setSelectedRole(selectedRole);
                    }
                })
                .catch(err => {
                    setErrorMessage("Не вдалося завантажити користувача. Перевірте правильність ID.");
                        ...
                        <div className="bg-white p-5 rounded-md shadow-md mb-8 mt-8 ">
                            <div className="sm:mx-auto sm:w-full sm:max-w-sm flex justify-center ">
                                <div className="pb-6 mt-6">
                                    <h2 className="text-base font-semibold leading-7 text-gray-900">{'Редагувати користувача'}</h2>
                                </div>
                            </div>
                                                        ...
                                                        <div className="col-span-12 sm:col-span-6">
                                                            <FormGroup>
                                                                <FormControlLabel
                                                                    control={
                                                                        <Checkbox
                                                                            checked={formData.generateNewPassword}
                                                                            onChange={(e) => setFormData(prevData => ({ ...prevData, generateNewPassword: e.target.checked }))}
                                                                            color="primary"
                                                                        />
                                                                    }
